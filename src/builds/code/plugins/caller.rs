...
mod tests {
    ...
    fn test_call_plugin_import_func_success() {
        ...
        let result = caller.call_plugin_import_func("rs", &current_path, &referenced_path, code_block);
        assert!(result.is_ok());
    ...
    fn test_multiple_plugins() {
        ...
        create_temp_plugin(temp_dir.path(), "rust", r#"
        function get_import_code(current_path, referenced_path, code_block)
        ...
        create_temp_plugin(temp_dir.path(), "python", r#"
        function get_import_code(current_path, referenced_path, code_block)
        ...
        let rust_import = caller.call_plugin_import_func(
            "rust", 
        ...
        let python_cleaned = caller.call_plugin_cleaning_func("python", "def main(): pass").unwrap();
