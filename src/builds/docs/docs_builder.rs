...
impl DocsBuilder {
    ...
    fn prepare_target_path(&self, path: &PathBuf) -> (PathBuf, String) {
        ...
        let extension = cleaned_res.extension().unwrap_or(std::ffi::OsStr::new("")).to_str().unwrap_or("").to_string();
    ...
    pub fn build(&self) -> Result<(), LPError> {
        for module in &self.project.modules {
            ...
            println!("Building docs for source: {:?} and target: {:?}", source_path, target_path);
            ...
            if let Some(sections) = &module.sections {
                ...
                std::fs::write(target_path, self.prepare_final_docs(sections, extension.as_str()))
